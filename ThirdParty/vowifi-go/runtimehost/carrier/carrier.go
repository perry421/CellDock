...
func NormalizeSubscriberProfile(in SubscriberProfileInput) SubscriberProfile {
	...
	if isDecimalString(imsi) && len(imsi) >= 3 {
		imsiMCC := normalizeMCC(imsi[:3])
		if mcc == "" || !strings.HasPrefix(imsi, mcc) {
			mcc = imsiMCC
		}
	}
	if isDecimalString(imsi) && !imsiMatchesMNC(imsi, mcc, mnc) {
		switch {
		case len(imsi) >= 6:
...
func imsiMatchesMNC(imsi, mcc, mnc string) bool {
	if imsi == "" || mcc == "" || mnc == "" || !strings.HasPrefix(imsi, mcc) {
		return false
	}
	rest := imsi[len(mcc):]
	...
	return len(mnc) == 3 && mnc[0] == '0' && strings.HasPrefix(rest, mnc[1:])
}
...
func normalizeNetworkConfig(mcc, mnc string, cfg NetworkConfig) NetworkConfig {
	...
	cfg.EAPMethod = normalizeEAPMethod(cfg.EAPMethod)
	if mcc == "" || mnc == "" {
...
func mergeNetworkAliasConfig(base, alias NetworkConfig) NetworkConfig {
	...
	if strings.TrimSpace(base.EAPMethod) == "" {
		base.EAPMethod = alias.EAPMethod
	}
	return base
}
