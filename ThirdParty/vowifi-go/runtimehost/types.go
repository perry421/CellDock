...
func buildTunnelConfig(req StartRequest, modem Modem) swu.TunnelConfig {
	...
	if req.Prepared != nil {
		...
		if strings.TrimSpace(req.Prepared.Profile.IMSI) != "" {
			cfg.IMSI = strings.TrimSpace(req.Prepared.Profile.IMSI)
		}
		if strings.TrimSpace(req.Prepared.Profile.MCC) != "" {
			cfg.MCC = strings.TrimSpace(req.Prepared.Profile.MCC)
		}
		if strings.TrimSpace(req.Prepared.Profile.MNC) != "" {
			cfg.MNC = strings.TrimSpace(req.Prepared.Profile.MNC)
		}
		if strings.TrimSpace(req.Prepared.Profile.IMEI) != "" {
		...
		cfg.APN = strings.TrimSpace(req.Prepared.CarrierPolicy.IMS.IMSAPN)
		cfg.EAPMethod = strings.TrimSpace(req.Prepared.CarrierPolicy.IMS.EAPMethod)
	}
	if cfg.APN == "" {
		cfg.APN = "ims"
	}
