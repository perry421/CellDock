...
func RunIKE_AUTH_Full(ctx context.Context, cfg FullAuthConfig) (FullAuthResult, error) {
	...
	for i := 0; i < maxFullAuthEAPExchanges; i++ {
		...
		if next.Code == eapaka.CodeSuccess {
			...
			finalBytes, finalInner, nextMessageID, err := runPostEAPAuth(ctx, cfg, out.EAPKeys, out.NextMessageID, auth)
			if err != nil {
				return out, err
			}
			out.FinalResponseBytes = finalBytes
			out.FinalResponseInner = finalInner
			out.NextMessageID = nextMessageID
			child, ok, err := parseChildSAIfPresent(cfg.Init, finalInner, localChildSPI, nextMessageID, offeredChildSA, offeredTSi, offeredTSr)
			if err != nil {
				return out, err
			}
			if !ok {
				return out, fmt.Errorf("%w: post-EAP AUTH response without CHILD_SA", ErrInvalidAuthResponse)
			}
			out.ChildSA = &child
			return out, nil
		}
...
var ikev2KeyPad = []byte("Key Pad for IKEv2")

// runPostEAPAuth completes the final IKE_AUTH round trip required after
// EAP-Success. RFC 5998 still requires AUTH in this exchange; EAP-only removes
// the responder's certificate authentication, not the AUTH values derived
// from the EAP MSK.
func runPostEAPAuth(ctx context.Context, cfg FullAuthConfig, eapKeys eapaka.Keys, messageID uint32, auth AuthResult) ([]byte, []Payload, uint32, error) {
	if len(eapKeys.MSK) == 0 {
		return nil, nil, messageID, fmt.Errorf("%w: EAP success without MSK", ErrInvalidAuthResponse)
	}
	keys := cfg.Keys
	if keys.Profile.RequiredLength() == 0 {
		keys = cfg.Init.Keys
	}
	idiBody, err := cfg.InitiatorID.MarshalBinary()
	if err != nil {
		return nil, nil, messageID, err
	}
	authData, err := calculateEAPAuth(keys.Profile.PRF, eapKeys.MSK, cfg.Init.RequestBytes, cfg.Init.NonceR, keys.SKPi, idiBody)
	if err != nil {
		return nil, nil, messageID, err
	}
	authBody := make([]byte, 4, 4+len(authData))
	authBody[0] = authMethodSharedKeyMIC
	authBody = append(authBody, authData...)
	iv, err := authIV(cfg.Random, keys.Profile, nil)
	if err != nil {
		return nil, nil, messageID, err
	}
	_, requestBytes, err := ProtectMessage(authHeader(cfg.Init, messageID, true), keys, true, []Payload{{Type: PayloadAUTH, Body: authBody}}, iv)
	if err != nil {
		return nil, nil, messageID, err
	}
	...
	_, inner, err := unprotectAuthResponse(responseBytes, cfg.Init, keys, messageID)
	if err != nil {
		return nil, nil, messageID, err
	}
	idr, err := responderIdentityForPostEAP(auth, cfg.ResponderID)
	if err != nil {
		return nil, nil, messageID, err
	}
	if err := verifyEAPAuthPayload(inner, cfg.Init, keys, eapKeys.MSK, idr); err != nil {
		return nil, nil, messageID, err
	}
	return append([]byte(nil), responseBytes...), clonePayloads(inner), messageID + 1, nil
}

func calculateEAPAuth(prfHash crypto.Hash, msk, ikeInitBytes, peerNonce, skP, idBody []byte) ([]byte, error) {
	idHash, err := PRF(prfHash, skP, idBody)
	if err != nil {
		return nil, err
	}
	signedOctets := make([]byte, 0, len(ikeInitBytes)+len(peerNonce)+len(idHash))
	signedOctets = append(signedOctets, ikeInitBytes...)
	signedOctets = append(signedOctets, peerNonce...)
	signedOctets = append(signedOctets, idHash...)
	key, err := PRF(prfHash, msk, ikev2KeyPad)
	if err != nil {
		return nil, err
	}
	return PRF(prfHash, key, signedOctets)
}

func responderIdentityForPostEAP(auth AuthResult, fallback Identity) (Identity, error) {
	for _, payload := range auth.InitialResponseInner {
		if payload.Type == PayloadIDr {
			return ParseIdentity(payload.Body)
		}
	}
	if fallback.Type != 0 {
		return fallback, nil
	}
	return Identity{}, fmt.Errorf("%w: post-EAP responder identity missing", ErrInvalidAuthResponse)
}

func verifyEAPAuthPayload(payloads []Payload, init InitResult, keys IKEKeys, msk []byte, idr Identity) error {
	var body []byte
	...
	if len(body) < 4 || body[0] != authMethodSharedKeyMIC {
		return fmt.Errorf("%w: post-EAP responder AUTH missing or has unsupported method", ErrInvalidAuthResponse)
	}
	idrBody, err := idr.MarshalBinary()
	if err != nil {
		return err
	}
	expected, err := calculateEAPAuth(keys.Profile.PRF, msk, init.ResponseBytes, init.NonceI, keys.SKPr, idrBody)
	if err != nil {
		return err
	}
	if !hmac.Equal(body[4:], expected) {
		return fmt.Errorf("%w: post-EAP responder AUTH verification failed", ErrInvalidAuthResponse)
	}
	return nil
}
...
func BuildIKEAuthInitialPayloads(cfg AuthConfig) ([]Payload, error) {
	...
	if cfg.ResponderID.Type != 0 {
		responderIDPayload, err := IdentityPayload(PayloadIDr, cfg.ResponderID)
		if err != nil {
			return nil, err
		}
		payloads = append(payloads, responderIDPayload)
	}
	childSA := cfg.ChildSA
	if len(childSA.Proposals) == 0 {
		...
		childSA = childSAWithSPI(childSA, cfg.ChildSPI)
	}
	...
	if len(tsi.Selectors) == 0 {
		tsi = DualStackAnyTrafficSelectors()
	}
	...
	if len(tsr.Selectors) == 0 {
		tsr = DualStackAnyTrafficSelectors()
	}
	...
	// RFC 7296 lists the child-SA negotiation before CP, and deployed ePDGs can
	// be stricter about this ordering than the base protocol requires. Android
	// IWLAN also advertises initial-contact, MOBIKE, and EAP-only authentication
	// in the protected IKE_AUTH request (not in IKE_SA_INIT).
	payloads = append(payloads, saPayload, tsiPayload, tsrPayload, cfgPayload)
	payloads = append(payloads, InitialContactNotify(), MOBIKESupportedNotify(), EAPOnlyAuthenticationNotify())
	return payloads, nil
}
...
func authOfferedChildSA(sa SecurityAssociation, localSPI []byte) SecurityAssociation {
	...
	return childSAWithSPI(sa, localSPI)
}

func childSAWithSPI(sa SecurityAssociation, spi []byte) SecurityAssociation {
	out := cloneSecurityAssociation(sa)
	for index := range out.Proposals {
		if len(out.Proposals[index].SPI) == 0 {
			out.Proposals[index].SPI = append([]byte(nil), spi...)
		}
	}
	return out
}
